function I_out=shift_according_to_deformation_3d(ux,uy,I_in)

%This moves the pixel values in I_in, according to ux,uy. If one pixel is reached multiple time, we average. 
%Intermediate pixels are filled with average of the surrounding.
...
if (size(ux,1)+size(uy,1)+size(I_in,1)~=3*size(ux,1) && size(ux,2)+size(uy,2)+size(I_in,2)~=3*size(ux,2) ) 
    'SIzes are not compatible'
    return
end

I_out(1:s_y,1:s_x,1:s_i,1)=0;
...
I_out=I_out(max_u+1:s_y-max_u,max_u+1:s_x-max_u,:,1);

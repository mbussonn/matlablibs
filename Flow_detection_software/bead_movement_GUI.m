function varargout = bead_movement_GUI(varargin)
% BEAD_MOVEMENT_GUI M-file for bead_movement_GUI.fig
%      BEAD_MOVEMENT_GUI, by itself, creates a new BEAD_MOVEMENT_GUI or raises the existing
%      singleton*.
%
%      H = BEAD_MOVEMENT_GUI returns the handle to a new BEAD_MOVEMENT_GUI or the handle to
%      the existing singleton*.
%
%      BEAD_MOVEMENT_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BEAD_MOVEMENT_GUI.M with the given input arguments.
%
%      BEAD_MOVEMENT_GUI('Property','Value',...) creates a new BEAD_MOVEMENT_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bead_movement_GUI_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to bead_movement_GUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help bead_movement_GUI

% Last Modified by GUIDE v2.5 13-Feb-2013 18:17:02

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @bead_movement_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @bead_movement_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bead_movement_GUI is made visible.
function bead_movement_GUI_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bead_movement_GUI (see VARARGIN)

% Choose default command line output for bead_movement_GUI
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes bead_movement_GUI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = bead_movement_GUI_OutputFcn(hObject, eventdata, handles) 
...
for i=1:gd:size(im,1)
    for j=1:gd:size(im,2)
        if BW2(i,j)==1
            ...
            if 1==0
                [ypeak, xpeak, max_c]=subpixel_detection(c,1)
            else

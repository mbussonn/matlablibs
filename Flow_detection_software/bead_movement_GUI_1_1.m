function varargout = bead_movement_GUI_1_1(varargin)
% BEAD_MOVEMENT_GUI_1_1 M-file for bead_movement_GUI_1_1.fig
%      BEAD_MOVEMENT_GUI_1_1, by itself, creates a new BEAD_MOVEMENT_GUI_1_1 or raises the existing
%      singleton*.
%
%      H = BEAD_MOVEMENT_GUI_1_1 returns the handle to a new BEAD_MOVEMENT_GUI_1_1 or the handle to
%      the existing singleton*.
%
%      BEAD_MOVEMENT_GUI_1_1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BEAD_MOVEMENT_GUI_1_1.M with the given input arguments.
%
%      BEAD_MOVEMENT_GUI_1_1('Property','Value',...) creates a new BEAD_MOVEMENT_GUI_1_1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before bead_movement_GUI_1_1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to bead_movement_GUI_1_1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help bead_movement_GUI_1_1

% Last Modified by GUIDE v2.5 12-Mar-2013 19:51:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @bead_movement_GUI_1_1_OpeningFcn, ...
                   'gui_OutputFcn',  @bead_movement_GUI_1_1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before bead_movement_GUI_1_1 is made visible.
function bead_movement_GUI_1_1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to bead_movement_GUI_1_1 (see VARARGIN)

% Choose default command line output for bead_movement_GUI_1_1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes bead_movement_GUI_1_1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = bead_movement_GUI_1_1_OutputFcn(hObject, eventdata, handles) 
...
handle.num_images=num_images;
    curr=round(get(handles.slider1,'Value'));
    i=curr;
    im=imread(d_path_f,i);
    im2=imread(d_path_f,i+1);
    if length(size(im))>2
        handles.im=im(:,:,get(handles.listbox_color,'Value'));      
        handles.im2=im2(:,:,get(handles.listbox_color,'Value'));
    else
    ...
    set(handles.slider1,'Max',num_images-1);
    slider_step(1) = 1/(num_images-1);
    slider_step(2) = 1/(num_images-1);
    set(handles.slider1,'sliderstep',slider_step);
...
for i=1:gd:size(im,1)
    ...
    for j=1:gd:size(im,2)
        if BW2(i,j)==1
            ...
            %now check if subpix is wanted. and then use the subfunction
            
            if (get(handles.checkbox_subpixel,'Value')==1 & max_c>=c_th)
                try
                    [ypeak, xpeak, max_c]=subpixel_detection(c,3);
            ...
            %check for errors
            if (max_c>1.1 | ypeak>(ss-gs) | xpeak>(ss-gs))
                [ypeak, xpeak] = ind2sub(size(c),imax(1));
                [max_c, imax] = max(c(:));
            end
...
[thet,vel]=cart2pol(x_v,y_v);
%remove null deformation
vel_f=vel;
vel_f(find(vel_f==0))=[]
thet_f=thet;
thet_f(find(vel==0))=[]

%get the velocoity in mu per seconds
vel_f=vel_f*str2num(get(handles.edit_mue2pix,'String'))/str2num(get(handles.edit_sec_per_frame,'String'));

axes(handles.edge);
[h_v,x_v_hist]=hist(vel_f,round(sqrt(length(vel_f))));
bar(x_v_hist,h_v/length(vel_f));
title('Velocity histogram')
xlabel('Velocity in µm/s')

axes(handles.gran);
title('Direction histogram')
rose(thet_f,round(sqrt(length(vel_f))));
...
overlay1 = imoverlay(im, bw5_perim, [1 0 0]);
...
%path='E:\Science\data\brian_stemer\temp1'
filter_versioninfo='advanced_display_and_filter_v1_5';
    ...
    k_size_temp=str2num(get(handles.edit_temp_kernel_size,'String'));
    k_sigma_temp=str2num(get(handles.edit_temp_sigma,'String'));
    max_flow_vel=str2num(get(handles.edit_max_flow_vel,'String'));
...
save([path,'\analysis_parameters.mat'],'filter_versioninfo','path','xcorr_thresh','k_size','k_sigma','k_size_temp','k_sigma_temp','mue2pix_ratio','x_shift','y_shift');
...
% --- Executes on button press in checkbox_subpixel.
function checkbox_subpixel_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_subpixel (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_subpixel
